 ...
 * @author lsx
 */
public class FixDexUtil {//这下面两个属性可自己修改
    //修复包含第一位是BugClass文件名
    private final static String REPAIR_FILE_NAME = "BugClass";
    //修复文件路径(默认初始路径为根目录)
    private final static String REPAIR_FILE_PATH = "1079";
    private final static String TAG = "sansuiban";
    ...
    public static void startRepair(final Context context) {
        ...
        File fileDir = externalStorageDirectory != null ? new File(externalStorageDirectory, FixDexUtil.REPAIR_FILE_PATH) : new File(context.getFilesDir(), FixDexUtil.DEX_DIR);// data/user/0/包名/files/odex（这个可以任意位置）
    ...
    public static boolean isGoingToFix(@NonNull Context context) {
        ...
        // 遍历所有的修复dex , 因为可能是多个dex修复包
        File fileDir = externalStorageDirectory != null ? new File(externalStorageDirectory, REPAIR_FILE_PATH) : new File(context.getFilesDir(), DEX_DIR);// data/data/包名/files/odex（这个可以任意位置）

        File[] listFiles = fileDir.listFiles();
        if (listFiles != null) {
            for (File file : listFiles) {
                if (file.getName().startsWith(REPAIR_FILE_NAME, 1) && (file.getName().endsWith(DEX_SUFFIX) || file.getName().endsWith(APK_SUFFIX) || file.getName().endsWith(JAR_SUFFIX) || file.getName().endsWith(ZIP_SUFFIX))) {
